...
import { TesterAccountSettingsSchema, ITesterAccountSettings } from "./schema";
...
const AccountSettings: FC<{ user: any }> = ({ user }) => {
  ...
  const form = useFormik<ITesterAccountSettings>({
    initialValues: {
    ...
    validationSchema: TesterAccountSettingsSchema,
    onSubmit: async (values) => {
      mutation.mutate(
        _.pick(values, ["fullName", "profileImageUrl", "phoneNumber"])
      );
    },
    onReset: () => {},
  });
      ...
      <form onSubmit={form.handleSubmit}>
        <div className='mt-[30px] mb-9 flex gap-x-6 mr-[30px]'>
          <div className='pl-4'>
            ...
            <p className='text-zinc-500 mt-1 text-center font-medium  leading-tight'>
              {form.values.type}
            </p>
          </div>
